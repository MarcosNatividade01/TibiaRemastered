...
function actions_keyCheck.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if (doorAccessUnlocked or player:getStorageValue(storage) == 1) and (doorAccessUnlocked or isInQuest < 1) then
		if iPos == pillar[1] or iPos == pillar[2] then
			if doorAccessUnlocked or player:getItemCount(29351) >= 2 then
