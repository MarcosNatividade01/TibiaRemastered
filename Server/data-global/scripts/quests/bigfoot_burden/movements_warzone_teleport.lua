...
function warzoneTeleport.onStepIn(creature, item, position, fromPosition)
	...
	for a = 1, #teleports do
		if player:getPosition() == Position(teleports[a].teleportPosition) then
			if not questAccessUnlocked and player:getStorageValue(Storage.Quest.U9_60.BigfootsBurden.Rank) < 1440 then
			...
			if not questAccessUnlocked and player:getStorageValue(Storage.Quest.U9_60.BigfootsBurden.WarzoneStatus) < 1 then
			...
			if not questAccessUnlocked and player:getStorageValue(teleports[a].storage) < 1 and not player:removeItem(16242, 1) then
