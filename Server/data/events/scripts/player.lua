...
function Player:onGainExperience(target, exp, rawExp)
	...
	if configManager.getBoolean(configKeys.PREY_ENABLED) then
		...
		if monsterType and monsterType:raceId() > 0 then
			local preyExperiencePercent = self:getPreyExperiencePercentage(monsterType:raceId())
	...
	-- Consume hunting-time timers after reading bonuses for this kill.
...
function Player:onCombat(target, item, primaryDamage, primaryType, secondaryDamage, secondaryType)
	...
	if damageMultiplier then
		local function scaleOffensiveDamage(damage, combatType)
			if damage < 0 and combatType ~= COMBAT_HEALING then
				return math.floor(damage * damageMultiplier)
			end
			return damage
		end

		primaryDamage = scaleOffensiveDamage(primaryDamage, primaryType)
		secondaryDamage = scaleOffensiveDamage(secondaryDamage, secondaryType)
	end
