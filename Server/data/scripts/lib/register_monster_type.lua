...
local SOLO_MONSTER_EXPERIENCE_MULTIPLIER = 1
local SOLO_BESTIARY_CHARM_POINTS_MULTIPLIER = 3
...
registerMonsterType.experience = function(mtype, mask)
	if mask.experience then
		mtype:experience(math.ceil(mask.experience * SOLO_MONSTER_EXPERIENCE_MULTIPLIER))
	end
end
...
registerMonsterType.Bestiary = function(mtype, mask)
	if mask.Bestiary then
		...
		if mask.Bestiary.CharmsPoints then
			mtype:BestiaryCharmsPoints(math.ceil(mask.Bestiary.CharmsPoints * SOLO_BESTIARY_CHARM_POINTS_MULTIPLIER))
		end
