...
function parseBuyStoreOffer(playerId, msg)
	...
	if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
		...
		if expBoostCount == -1 or expBoostCount == 0 or expBoostCount > GameStore.ItemLimit.EXPBOOST then
			expBoostCount = 1
		end
		offerPrice = GameStore.ExpBoostValues[expBoostCount] or offer.price
	end
		...
		if count > 0 then
			...
			for _, off in ipairs(offer.offers) do
				...
				if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
					...
					xpBoostPrice = GameStore.ExpBoostValues[expBoostCount] or offer.price
				end
	...
	for _, offer in ipairs(category.offers) do
		if count > 0 and offer.coinType == GameStore.CoinType.Coin then
			...
			if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
				...
				offerPrice = GameStore.ExpBoostValues[expBoostCount] or offerPrice
			end
...
function Player.makeCoinTransaction(self, offer, desc)
	...
	local expBoostCount = nil
	if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
		expBoostCount = self:getStorageValue(GameStore.Storages.expBoostCount)
		if expBoostCount == -1 or expBoostCount == 0 or expBoostCount > GameStore.ItemLimit.EXPBOOST then
			expBoostCount = 1
		end
		offer.price = GameStore.ExpBoostValues[expBoostCount] or offer.price
	end
